...
class SecondWindow(QWidget,QApplication):
    def __init__(self, file_path):
        ...
        """)
        self.OrdenarPor_combobox.setFixedSize(200, 50)  # Fijar tamaño del ComboBox
    ...
    def update_table(self):
        ...
        while paletas_agregadas < int(self.line_edit.text()):
            ...
            factor_tm_pl = float(df_no_procesados.at[min_value_row_idx, 'Factor TM/PL'])
            inv_en_origen_tm = float(df_no_procesados.at[min_value_row_idx, 'Inv Final en Origen TM'])
            target = float(df_no_procesados.at[min_value_row_idx, 'Target de Inventario'])
            ...
            for idx, row in filtered_df.iterrows():
                ...
                try:   
                    ...
                    # Nuevo Porcentaje %
                    nuevo_percent = ((transit_tm + planned_tm + inv_exist_tm + Paleta_a_Tm) / target_inv) * 100
                    ...
                    filtered_df.at[idx, '% Con Corrección dupli'] = nuevo_percent
                    filtered_df.at[idx, '% Target Original'] = TargetOriginal
                    filtered_df.at[idx, 'Inv Final en Origen TM'] = round(origen-Paleta_a_Tm,5)
                    filtered_df.at[idx, '% Target Original dupli'] = TargetOriginal
                except: 
                    pass
    ...
    def calculo_Manual(self, row2, column):  
        if(column == 20):
            ...
            self.resumen.setText(f"Total paletas a enviar: {self.tm_estandar+self.suma_total}")          
        else: 
            pass
    ...
    def guardarSj(self):
        try:
            ...
            if not selected_localidad or not selected_categoria:
                self.mostrarMensaje("Error", "Debe seleccionar una localidad y una categoría.", tipo='error')
                return
            ...
            if 'localidad' not in column_names or 'categoria' not in column_names:
                self.mostrarMensaje("Error", "Las columnas 'localidad' y 'categoria' no están presentes en la tabla.", tipo='error')
                return
                            ...
                            if item:
                                ...
                                if col == 3:
                                    ...
                                    if item_col18 and item_col14:
                                        valor=(float(item_col15.text()) + float(item_col18.text()))
                                        valor2=int(valor * float(item_col14.text()))
            ...
            columns = ['Sku', 'Cantidad', 'Precio total', 'UOM Prim', 'Branchplant Origen', 'Branchplant Destino']
            df = pd.DataFrame(filtered_data, columns=columns)
            timestamp = datetime.now().strftime('%Y%m%d%H%M')
            file_name = f'Subida_Sj_{selected_localidad}_{selected_categoria}_{timestamp}.xlsx'
